...
// TestGetCaptureStatsEmptyDirectories tests stats with empty directories
func TestGetCaptureStatsEmptyDirectories(t *testing.T) {
	// Create temporary directory
	...
	assert.Equal(t, int64(0), stats.ScreenshotTotalSize)
	assert.Equal(t, int64(0), stats.PageSourceTotalSize)
