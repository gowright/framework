...
func TestResourceManager_testNeedsBrowser(t *testing.T) {
	...
	assert.False(t, rm.testNeedsBrowser(uiTest)) // Updated to match current implementation
...
func TestResourceManager_testNeedsHTTPClient(t *testing.T) {
	...
	// Test with API test
	apiTest := &MockAPITest{Name: "api_test"}
	assert.False(t, rm.testNeedsHTTPClient(apiTest)) // Updated to match current implementation
